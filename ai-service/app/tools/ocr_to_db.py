...
def get_or_create_subject(conn, subject_name: str) -> str:
    with conn.cursor() as cur:
        cur.execute("SELECT subject_id FROM subjects WHERE name = %s", (subject_name,))
        row = cur.fetchone()
        if row:
            return row[0]
        new_id = str(uuid.uuid4())
        cur.execute("INSERT INTO subjects (subject_id, name) VALUES (%s, %s)", (new_id, subject_name))
        return new_id

def get_or_create_topic(conn, subject_id: str, topic_name: str) -> str:
    with conn.cursor() as cur:
        cur.execute("SELECT topic_id FROM topics WHERE subject_id = %s AND name = %s", (subject_id, topic_name))
        row = cur.fetchone()
        if row:
            return row[0]
        new_id = str(uuid.uuid4())
        cur.execute("INSERT INTO topics (topic_id, subject_id, name) VALUES (%s, %s, %s)", (new_id, subject_id, topic_name))
        return new_id

def get_difficulty_id(conn, diff_name: str) -> str:
    with conn.cursor() as cur:
        cur.execute("SELECT difficulty_id FROM difficulties WHERE name = %s", (diff_name,))
        row = cur.fetchone()
        if row:
            return row[0]
        new_id = str(uuid.uuid4())
        cur.execute("INSERT INTO difficulties (difficulty_id, name) VALUES (%s, %s)", (new_id, diff_name))
        return new_id

def insert_into_db(conn, rows: list[dict]) -> None:
    """Insert parsed questions and relationships into the database."""
    print(f"  Inserting {len(rows)} questions into the database...")
    for row in rows:
        subject_id = get_or_create_subject(conn, row["subject"])
        topic_id = get_or_create_topic(conn, subject_id, row["topic"])
        difficulty_id = get_difficulty_id(conn, row["difficulty"])
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (new_q_id, row["pyp_id"], row["target_text"], marks)
            )
            question_id = new_q_id
            
            # Link question to topic and difficulty
            cur.execute(
                "INSERT INTO question_topics (question_id, topic_id, difficulty_id) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
                (question_id, topic_id, difficulty_id)
            )
    conn.commit()
    print("  [OK] Database insertion complete.")
...
def process_paper(conn, paper: dict, config: dict, env: dict, topic_keywords: dict) -> bool:
    """
    ...
    print(f"  Parsed questions generated: {len(rows)}")
...
def main():
    ...
    for paper in papers:
        ok = process_paper(conn, paper, config, env, topic_keywords)
        if ok:
            succeeded += 1
